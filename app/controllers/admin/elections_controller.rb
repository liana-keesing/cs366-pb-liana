...
=end
    ...
    # our method!
    def compute_and_fund_projects_by_equal_shares(projects, voters, total_budget)
      puts "Initial Budget: #{total_budget}" # Debug print
      shares = Hash[voters.map { |v| [v.id, total_budget.to_f / voters.size] }]
      funded_projects = {}
      projs = projects.dup
      while projs.any?
        # Filter projects based on whether their total cost can be covered by the shares of voters who voted for them
        projs = projs.select do |project|
          project_voters = VoteKnapsack.where(project_id: project.id)
                                        .joins(:voter)
                                        .where(voters: { void: 0 })
                                        .pluck(:voter_id)
